...
class TestBuildSystemPrompt:
    ...
    def test_session_metadata_kept_out_of_system_prompt(self, agent):
        prompt = agent._build_system_prompt()
        # Per-session metadata is dynamic, so it must not alter the cached
        # system-prompt prefix. It rides on the current user message instead.
        assert "Conversation started:" not in prompt
        assert "Conversation started:" in agent._session_metadata_line

    def test_session_metadata_line_carries_runtime_identity(self, agent):
        """Timestamp/model/provider metadata lives in the user-message
        injection sidecar, not in the stable system prompt."""
        agent.model = "test-model"
        agent.provider = "openrouter"
        prompt = agent._build_system_prompt()
        metadata = agent._session_metadata_line

        assert "Conversation started:" in metadata
        assert "Conversation started:" not in prompt
        assert "Model: test-model" in metadata
