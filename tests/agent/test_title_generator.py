...
class TestGenerateTitle:
    ...
    def test_extract_mode_skips_llm_and_strips_platform_prefix(self):
        ...
        with (
            ...
            title = generate_title("[Patryk] Zaktualizować Hermesa. Drugi temat", "OK")

        assert title == "Zaktualizować Hermesa"
        mock_call_llm.assert_not_called()

    def test_falls_back_to_extraction_on_exception(self):
        with patch("agent.title_generator.call_llm", side_effect=RuntimeError("no provider")):
            assert generate_title("question", "answer") == "question"
    ...
    def test_no_callback_falls_back_to_extraction(self):
        with patch("agent.title_generator.call_llm", side_effect=RuntimeError("nope")):
