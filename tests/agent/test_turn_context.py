...
from agent.turn_context import TurnContext, build_turn_context, compose_user_api_content
...
def test_compose_user_api_content_injects_dynamic_context_after_clean_content():
    composed = compose_user_api_content(
        "hello",
        ext_prefetch_cache="remembered fact",
        plugin_user_context="plugin note",
        session_metadata_line="Conversation started: Monday\nModel: local",
    ...
    assert "\n\nephemeral prompt\n\n" in composed
    assert "remembered fact" in composed
    assert composed.endswith("plugin note")


def test_turn_start_stamps_api_content_without_mutating_clean_user_content():
    agent = _FakeAgent()
    agent._session_metadata_line = "Conversation started: Monday"
    agent.ephemeral_system_prompt = "ephemeral prompt"
    ...
    assert "ephemeral prompt" in api_content


def test_multimodal_content_is_not_string_sidecar_composed():
