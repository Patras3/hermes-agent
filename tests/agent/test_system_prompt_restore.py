...
class TestStoredPromptReuse:
    ...
    def test_present_row_with_stale_row_model_rebuilds_without_prompt_metadata(self, caplog):
        """New cache-friendly prompts omit Model:/Provider: lines.

        Runtime identity for those prompts must be validated against the
        session row metadata, otherwise a /model switch can restore an old
        model-specific prompt forever because the prompt itself has no footer to
        compare.
        """
        stored = "You are Hermes Agent.\n\nNo runtime metadata in this prompt."
        db = MagicMock()
        ...
        agent = _make_agent(session_db=db, prebuilt_prompt="REBUILT for new model")
        agent.model = "openai/gpt-5.5"

        with caplog.at_level(logging.INFO, logger="agent.conversation_loop"):
            _restore_or_build_system_prompt(agent, None, [{"role": "user", "content": "hi"}])

        assert agent._cached_system_prompt == "REBUILT for new model"
        agent._build_system_prompt.assert_called_once_with(None)
        db.update_system_prompt.assert_called_once_with(
            agent.session_id, agent._cached_system_prompt
        )
    ...
    def test_restore_refreshes_runtime_metadata_sidecar(self):
        """Restoring prompt bytes must still refresh current Model:/Provider: metadata."""
        stored = "Stored prompt without runtime footer"
        db = MagicMock()
        db.get_session.return_value = {
            "system_prompt": stored,
            "model": "test-model",
        ...
        assert agent._cached_system_prompt == stored
        assert "Model: test-model" in agent._session_metadata_line
        assert "Provider: openrouter" in agent._session_metadata_line
        assert "Model: test-model" not in agent._cached_system_prompt
