...
export function DesktopInstallOverlay({ enabled = true }: DesktopInstallOverlayProps) {
  ...
  const [localStartingRoot, setLocalStartingRoot] = useState<string | null>(null)
  const [localStartError, setLocalStartError] = useState<{ root: string; message: string } | null>(null)
  ...
  // a manifest/failure takes ownership, clear the transient button state so it
  // cannot leak into a later setup-choice screen. Do not clear on the first
  // async snapshot: a fast click in tests/real UI can race that initial effect.
  useEffect(() => {
    if (!setupChoiceRoot) {
    ...
    const lastSetupChoiceRoot = lastSetupChoiceRootRef.current
    lastSetupChoiceRootRef.current = setupChoiceRoot

    if (lastSetupChoiceRoot && lastSetupChoiceRoot !== setupChoiceRoot) {
      setLocalStartingRoot(null)
      setLocalStartError(null)
    }
  }, [setupChoiceRoot])
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            ...
            <button
              ...
              onClick={async () => {
                ...
                setLocalStartingRoot(choiceRoot)
                setLocalStartError(null)
                ...
                try {
                  ...
                  setLocalStartError({ root: choiceRoot, message: errorMessage(err) })
                  setLocalStartingRoot(current => (current === choiceRoot ? null : current))
                }
              }}
              type="button"
            >
