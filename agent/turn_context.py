...
def compose_user_api_content(
    ...
    session_metadata_line: str = "",
    ephemeral_system_prompt: str = "",
) -> Optional[str]:
    """Compose the API-bound content of the current turn's user message.

    Sources: session metadata, ephemeral system prompt, memory-manager
    prefetch, and ``pre_llm_call`` plugin context with target="user_message"
    (the default). All are appended to the *API copy* of the user message only
    ...
    if not isinstance(content, str):
        return None
    injections = []
    if session_metadata_line:
        injections.append(f"[{session_metadata_line}]")
    if ephemeral_system_prompt:
        injections.append(ephemeral_system_prompt)
...
def build_turn_context(
    ...
    *,
    ...
    # The session metadata / ephemeral prompt / prefetch / plugin context above
    # is injected into the API copy of this turn's user message, never into the
    # stored clean content or cached system prompt — so on the next turn the
    # message would replay WITHOUT the injection, diverging the request prefix at
    # this point and re-prefilling everything after it (the whole previous turn's
    # assistant/tool chain). Stamp the exact API-bound bytes on the live dict,
    # only when they differ from the clean content, so the crash persist below
    # writes both in the same row and replay can reproduce the sent prefix
    # byte-for-byte. Guarded by the same predicate the api_messages build uses,
