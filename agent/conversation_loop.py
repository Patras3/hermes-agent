...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt, session_row):
        # Continuing session — reuse the exact system prompt from the
        # previous turn so the Anthropic cache prefix matches.
        agent._cached_system_prompt = stored_prompt
        try:
            from agent.system_prompt import refresh_session_metadata_line as _refresh_meta

            _refresh_meta(agent)
        except Exception:
            logger.debug("session metadata sidecar refresh failed on restore", exc_info=True)
...
def _stored_prompt_matches_runtime(agent, prompt: str, session_row: Any = None) -> bool:
    """Return False when persisted runtime identity is stale.

    Older stored prompts embedded ``Model:`` / ``Provider:`` lines directly.
    Newer prompts keep that metadata out of the cached system prompt, so the
    session row's model/billing metadata is the only out-of-band stale-prompt
    guard after a /model switch.
    """
    ...
    row_model = row_value("model")
    if row_model and current_model and row_model != current_model:
        return False

    row_provider = row_value("billing_provider")
    # Billing buckets for custom providers may be stored as bare "custom" while
    # the live provider is the routable "custom:<name>". Treat that as a match;
    # the precise custom identity lives in model_config and session overrides.
    if (
        row_provider
        and current_provider
        and row_provider != current_provider
        and not (row_provider == "custom" and current_provider.startswith("custom:"))
    ):
        return False
...
def _replace_metadata_block(sidecar: str, clean_content: str, metadata_line: str) -> str:
    """Replace/prepend the dynamic runtime metadata block in api_content."""
    if not isinstance(sidecar, str) or not isinstance(clean_content, str):
        return sidecar
    ...
    tail = sidecar[len(prefix):]
    if tail.startswith("[Conversation started:"):
        marker = "]\n\n"
        end = tail.find(marker)
        if end >= 0:
            return prefix + metadata_block + tail[end + 1:]
        if tail.endswith("]"):
            return prefix + metadata_block
    return prefix + metadata_block + "\n\n" + tail


def _sync_failover_system_message(
    agent,
    api_messages,
    ...
    """Refresh in-flight request identity after provider failover.

    Older stored prompts embed ``Model:``/``Provider:`` lines in the cached
    system prompt; modern cache-stable prompts keep runtime identity in the
    current user message's ``api_content`` sidecar. A fallback changes the
    active runtime after ``api_messages`` were built, so retrying the same list
    must patch both places: system for legacy prompts and sidecar/wire content
    for current prompts.

    Mutates ``api_messages`` (and, when provided, the live current-turn
    ``conversation_messages`` sidecar) in place and returns the prompt to use
    as ``active_system_prompt`` for subsequent call-block rebuilds.
    """
    try:
        from agent.system_prompt import refresh_session_metadata_line

        refresh_session_metadata_line(agent)
    except Exception:
        logger.debug("failover metadata refresh failed", exc_info=True)

    sp = getattr(agent, "_cached_system_prompt", None)
    if not isinstance(sp, str) or not sp:
        return active_system_prompt
    if api_messages and api_messages[0].get("role") == "system":
        api_messages[0]["content"] = sp

    metadata_line = getattr(agent, "_session_metadata_line", "") or ""
    if (
        metadata_line
        and isinstance(conversation_messages, list)
        and isinstance(current_turn_user_idx, int)
        and 0 <= current_turn_user_idx < len(conversation_messages)
    ):
        try:
            live_msg = conversation_messages[current_turn_user_idx]
            if isinstance(live_msg, dict) and live_msg.get("role") == "user":
                clean = live_msg.get("content", "")
                sidecar = live_msg.get("api_content")
                if isinstance(sidecar, str) and isinstance(clean, str):
                    updated = _replace_metadata_block(sidecar, clean, metadata_line)
                    live_msg["api_content"] = updated
                    for api_msg in reversed(api_messages):
                        if api_msg.get("role") == "user" and isinstance(api_msg.get("content"), str):
                            api_msg["content"] = updated
                            break
        except Exception:
            logger.debug("failover api_content metadata sync failed", exc_info=True)
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
            ...
            if idx == current_turn_user_idx and msg.get("role") == "user":
                if isinstance(_api_content, str) and _api_content:
                    ...
                    _composed = compose_user_api_content(
                        ...
                        getattr(agent, "_session_metadata_line", "") or "",
        ...
        # stay warm.
        # Keep dynamic per-session/system-overlay text out of the system prompt;
        # it is delivered on the current user message via compose_user_api_content
        # so the cached system-prompt prefix remains byte-stable.
