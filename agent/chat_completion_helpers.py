...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Keep the prompt/runtime identity in sync with the model actually
        # answering, so "what model are you?" doesn't report the primary.
        # Modern cache-stable prompts carry runtime identity in the per-turn
        # user-message sidecar; rewrite_prompt_model_identity is retained for
        # older stored prompts that still have Model/Provider lines in system.
        ...
        try:
            ...
            logger.debug("failed to refresh fallback session metadata", exc_info=True)
