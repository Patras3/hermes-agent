...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Session metadata (timestamp, session id, model, provider) is injected into
    # the current user turn instead of the system prompt so the system-prompt
    # prefix stays byte-identical across sessions. This preserves provider / KV
    # prompt-cache reuse on single-GPU local runtimes where changing even a late
    # system-prompt suffix can force a full prefill.
...
def refresh_session_metadata_line(agent: Any) -> None:
    """Refresh the per-turn runtime metadata sidecar.

    This intentionally lives outside the cached/stored system prompt. Restore
    paths can call it without rebuilding or mutating the persisted prompt bytes,
    while still keeping Model:/Provider: identity current after /model switches.
    """
    ...
    if getattr(agent, "pass_session_id", False) and getattr(agent, "session_id", None):
        _meta_parts.append(f"Session ID: {agent.session_id}")
    if getattr(agent, "model", None):
        _meta_parts.append(f"Model: {agent.model}")
    if getattr(agent, "provider", None):
        _meta_parts.append(f"Provider: {agent.provider}")
