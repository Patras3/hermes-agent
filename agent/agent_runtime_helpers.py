...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        # Undo the fallback identity update so the prompt is byte-identical to
        # the stored copy again (prefix cache match) and the dynamic metadata
        # sidecar points back at the restored primary runtime.
        ...
        try:
            ...
            logger.debug("failed to refresh restored-primary session metadata", exc_info=True)
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # Clear both the full cached prompt and the separately-tracked static
    # prefix; otherwise a /model switch can pair a fresh prompt with the old
    # early cache breakpoint until the next explicit invalidation.
