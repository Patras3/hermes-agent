...
def _title_config() -> dict:
    """Return the read-only title generation config."""
    try:
        # Lazy import: title_generator is imported from agent code paths where a
        # module-level hermes_cli import risks circularity, and the read-only
        # loader avoids config-migration writes from the title background thread.
        from hermes_cli.config import load_config_readonly

        config = load_config_readonly()
        title_config = (config.get("auxiliary") or {}).get("title_generation") or {}
        return title_config if isinstance(title_config, dict) else {}
    except Exception:
        logger.debug("Failed to read auxiliary.title_generation config", exc_info=True)
        return {}
...
def _title_from_message(user_message: str) -> Optional[str]:
    ...
    # Take the first sentence or the first 50 chars.
    for sep in (".", "!", "?", "\n"):
        idx = text.find(sep)
        if 0 < idx <= 60:
            text = text[:idx]
...
def _title_mode() -> str:
    """Return the configured title mode; "extract" skips the LLM call."""
    return str(_title_config().get("mode", "")).strip().lower()
...
def generate_title(
    ...
    When ``auxiliary.title_generation.mode`` is set to ``"extract"``, skip the
    LLM call entirely and derive the title from the beginning of the user
    message — zero latency and no second model load on single-GPU setups.

    Otherwise uses the main runtime's model when available, falling back to the
