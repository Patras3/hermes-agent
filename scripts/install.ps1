    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Warn "Could not install [web] extra. Run manually: uv pip install --python `"$pythonExe`" `"fastapi==0.133.1`" `"uvicorn[standard]==0.41.0`" `"starlette==1.3.1`" `"python-multipart==0.0.31`""
            }
        }
...
function Install-PlatformSdks {
    ...
    $sdkMap = @(
        ...
        @{ Var = "SLACK_BOT_TOKEN";    Import = "slack_sdk"; Spec = "slack-sdk==3.43.0" },
        @{ Var = "SLACK_APP_TOKEN";    Import = "slack_bolt";Spec = "slack-bolt==1.29.0" },
