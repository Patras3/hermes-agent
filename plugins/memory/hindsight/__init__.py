...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        # Step 2: Install exact, reviewed deps for selected mode. Keep these
        # pins in sync with tools/lazy_deps.py and plugin.yaml; avoiding
        # open-ended upgrades matters for optional setup-time install paths too.
        cloud_dep = f"hindsight-client=={_MIN_CLIENT_VERSION}"
        local_dep = f"hindsight-all=={_MIN_CLIENT_VERSION}"
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                if uv_path:
                    try:
                        ...
                        logger.info("hindsight-client pinned to %s", _MIN_CLIENT_VERSION)
                    except Exception as e:
